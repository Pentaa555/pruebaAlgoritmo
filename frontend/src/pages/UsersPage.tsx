  ...
  return (
    <div style={{ padding: '2rem' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
        <h1 style={{ color: '#1e3a8a', margin: 0 }}>Usuarios</h1>
        <Link to="/users/new" style={{ background: '#2563eb', color: '#fff', padding: '8px 16px', borderRadius: '4px', textDecoration: 'none', fontSize: '14px' }}>+ Nuevo usuario</Link>
      </div>
      ...
      {loading ? <p style={{ color: '#6b7280' }}>Cargando...</p> : (
        <div style={{ overflowX: 'auto' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '14px' }}>
            ...
            <tbody>
              {data?.items.length === 0 && (
                <tr><td colSpan={5} style={{ padding: '24px', textAlign: 'center', color: '#6b7280' }}>No se encontraron usuarios.</td></tr>
              )}
              {data?.items.map((u) => (
                <tr key={u.id} style={{ borderBottom: '1px solid #f1f5f9' }}>
                  ...
                  <td style={{ padding: '10px 12px', color: u.isActive ? '#16a34a' : '#dc2626' }}>{u.isActive ? 'Sí' : 'No'}</td>
                  <td style={{ padding: '10px 12px', display: 'flex', gap: '12px' }}>
                    <Link to={`/users/${u.id}/edit`} style={{ color: '#2563eb', textDecoration: 'none', fontSize: '13px' }}>Editar</Link>
                    <button onClick={() => handleDelete(u.id)} style={{ color: '#dc2626', background: 'none', border: 'none', cursor: 'pointer', padding: 0, fontSize: '13px' }}>Eliminar</button>
                  </td>
                </tr>
              ))}
            </tbody>
